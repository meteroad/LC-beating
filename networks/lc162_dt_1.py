...
class DilatedMultiheadSelfAttentionWithRelativePositionalEmbedding(nn.Module):
    ...
    def kv_roll(self, tensor, layer, padding_value=0):
        ...
        dilation_list = []
        # right kernel
        dilation_list.append(-self.latency)
        # left kernel
...
class MainModule(nn.Module):
    def __init__(self, attn_len=5, ntoken=2, dmodel=64, nhead=8, d_hid=64, nlayers=11, norm_first=True, dropout=.1, latency=160):
        ...
        for idx in range(nlayers):
            layer_latency = round(unused_latency/(nlayers-idx))
            self.Transformer_layers[f'time_attention_{idx}'] = DilatedTransformerLayer(dmodel, nhead, d_hid, dropout, Er_provided=False, attn_len=attn_len, norm_first=norm_first, latency=layer_latency)
