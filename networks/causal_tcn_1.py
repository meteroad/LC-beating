...
# brief introduction of the changed model
# 倾斜金字塔型的TCN, 进行了一些计算上的简化, 最小latency是1。

class defConv2(nn.Module):
    def __init__(self, in_channels, out_channels, left_kernel=5, latency=10, dilation=2, bias=True):
        super().__init__()
        ...
        dilation_list = []
        # right kernel
        dilation_list.append(-round((1+log2(dilation))/11*latency))
        # left kernel
